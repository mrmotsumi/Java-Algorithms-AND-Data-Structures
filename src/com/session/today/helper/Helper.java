...
public class Helper {
	...
	public static void QuickSortStep(int[] database, int leftBound, int rightBound) {
		...
		if(leftBound >= rightBound) return; // indices have crossed;
		...
		while(leftInd <= rightInd) { // scan right until larger than the pivot
			...
			while((leftInd <= rightInd) &&  (database[leftInd] <= pivot)) { leftInd++;}
			
			while((rightInd >= leftInd) && (database[rightInd] >= pivot)) {rightInd--;}
			
			
			if(leftInd < rightInd) {  // both elements were found so swap
				
				Helper.Swap(database, leftInd, rightInd);
				//temp = database[rightInd]; database[rightInd] = database[leftInd]; database[leftInd] = temp;
				
		        Main.PrintHorizontalArray(rightInd,  database.length-1 ); // print the array to show how many steps taken to solve the problem
